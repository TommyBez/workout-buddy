...
import { workoutPlanOutputSchema } from "@/lib/schemas"

export async function POST(req: Request) {
  const body = await req.json()
  const { goal, metrics, preferences, feedback, recentLogs } = body
...
${feedback ? `\nUser feedback on their current plan: "${feedback}"\nAdjust the plan based on this feedback. Make iterative improvements rather than starting from scratch.` : ""}

${recentLogs ? `\nRecent workout data: The user has completed ${recentLogs.count} workouts with an average difficulty rating of ${recentLogs.avgDifficulty}/5. ${recentLogs.avgDifficulty >= 4 ? "Consider reducing volume or intensity slightly." : recentLogs.avgDifficulty <= 2 ? "Consider increasing volume or intensity." : "Current difficulty seems appropriate."}` : ""}`
